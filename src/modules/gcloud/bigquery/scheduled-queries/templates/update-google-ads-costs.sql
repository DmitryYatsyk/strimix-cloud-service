...
source_match_keys as (
  select
    concat(cast(date as string), ad_account_id, ifnull(trim(source), '_'), ifnull(trim(medium), '_'), ifnull(trim(campaign), '_'), ifnull(trim(content), '_'), ifnull(trim(term), '_'), ifnull(trim(strimix_refid), '_'), ad_id, ifnull(trim(keyword), '_')) match_key,
    *
